...
export const generateCountryCodes: country[] = countryCodes.map(c => {
  let obj: country = {} as country;
  obj.value = c.iso3_cc == '' ? c.iso2_cc : c.iso3_cc;
  obj.label = c.name;
  return obj;
});

export const generateNationalityValue = (nationality: string): string => {
  if (nationality) {
    let country = generateCountryCodes?.find(
...
export const generateSelectedCountry = (value: string): string => {
  let country = countryCodes?.find(country => country.name == value);
  return country!.iso3_cc == '' ? country!.iso2_cc : country!.iso3_cc;
};
